...
'/software/components/filecopy/services' = {
    if ( length(PROOF_SERVICE) > 0 ) {
        ...
        SELF[escape(PROOF_CHECKDAEMONS_FILE)] =  dict('config', contents,
            'owner', 'root:root',
            'perms', '0755'
        );
    };
...
"/software/components/cron/entries" = {
    if ( is_defined(PROOF_CHECKDAEMONS_INTERVAL) && (PROOF_CHECKDAEMONS_INTERVAL > 0) ) {
        SELF[length(SELF)] = dict("name", PROOF_CHECKDAEMONS_SCRIPT,
            "user", "root",
...
"/software/components/altlogrotate/entries" = {
    if ( is_defined(PROOF_CHECKDAEMONS_INTERVAL) && (PROOF_CHECKDAEMONS_INTERVAL > 0) ) {
        SELF[PROOF_CHECKDAEMONS_SCRIPT] = dict("pattern", "/var/log/"+PROOF_CHECKDAEMONS_SCRIPT+".ncm-cron.log",
            "compress", true,
            "missingok", true,
            "frequency", "weekly",
            "create", true,
